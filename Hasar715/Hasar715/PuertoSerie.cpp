...
PuertoSerie::PuertoSerie(unsigned nPuerto, unsigned Velocidad, Paridades Paridad, unsigned char BitsStop, unsigned char BitsDatos) throw(Excepcion)
{
	Debug::Debug("PuertoSerie", "Open COM%u, %u bauds", nPuerto, Velocidad);
	...
	if (i == Handles.end())
	{
		...
		if (!(Port.Overlapped.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL)))
		{
			...
			Debug::Debug("PuertoSerie", "Error CreateEvent: %s", ErrDesc.c_str());
		...
		if (Port.Handle == INVALID_HANDLE_VALUE)
		{
			...
			Debug::Debug("PuertoSerie", "Error open serial %s: %s", ComName, ErrDesc.c_str());
	...
	{
		Debug::Debug("PuertoSerie", "Reopen");
		Port = i->second;
		Port.Count++;
	}
	...
	{
		std::string ErrDesc = Excepcion::GetLastErrorDesc();
		Debug::Debug("PuertoSerie", "GetCommState: %s", ErrDesc.c_str());
		Close();
	...
	{
		std::string ErrDesc = Excepcion::GetLastErrorDesc();
		Debug::Debug("PuertoSerie", "SetCommState: %s", ErrDesc.c_str());
		Close();
		throw Excepcion(Excepcion::TRANSPORTE_ERROR_PARAMETROS_SERIE, ErrDesc);
	}
	...
	{
		std::string ErrDesc = Excepcion::GetLastErrorDesc();
		Debug::Debug("PuertoSerie", "Error open: %s", ErrDesc.c_str());
		throw Excepcion(Excepcion::TRANSPORTE_ERROR_APERTURA, ErrDesc);
	}
	...
	{
		std::string ErrDesc = Excepcion::GetLastErrorDesc();
		Debug::Debug("PuertoSerie", "tcgetattr: %s", ErrDesc.c_str());
		Close();
		throw Excepcion(Excepcion::TRANSPORTE_ERROR_APERTURA, ErrDesc);
	}
	cfmakeraw(&Params);
	...
	{
		std::string ErrDesc = Excepcion::GetLastErrorDesc();
		Debug::Debug("PuertoSerie", "tcsetattr: %s", ErrDesc.c_str());
		Close();
		throw Excepcion(Excepcion::TRANSPORTE_ERROR_PARAMETROS_SERIE, ErrDesc);
	}
#endif
...
PuertoSerie::Close()
{
	...
	if (i != Handles.end())
	{
		...
		if (!--p.Count)
		{
			Debug::Debug("PuertoSerie", "Close COM%u", i->first);
