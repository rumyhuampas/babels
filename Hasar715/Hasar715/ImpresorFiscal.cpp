...
ImpresorFiscal::ImpresorFiscal(Protocolos Prot)
{
	if (Prot == PROTOCOLO_STATPRN)
		Protocolo = new ProtocoloStatPRN;
	else
		Protocolo = new ProtocoloOriginal;
...
ImpresorFiscal::EstablecerPuertoSerie(unsigned nPuerto) throw(Excepcion)
{
	...
	for (i = 0; i < MAX_VELOCIDADES; i++)
	{
		...
		try
		{
			Protocolo->EstablecerTransporte(new PuertoSerie(nPuerto, Velocidades[i]));
			ProbarTransporte();
			break;
		}
		catch(Excepcion &e)
...
ImpresorFiscal::EstablecerPuertoSerie  (unsigned nPuerto,
										...
										throw(Excepcion)
{
	Protocolo->EstablecerTransporte(new PuertoSerie (nPuerto, Velocidad));
	ProbarTransporte();
}
...
ImpresorFiscal::EstablecerSocketTCP    (const std::string &DireccionRemota,
										...
										throw(Excepcion)
{
	Protocolo->EstablecerTransporte(new SocketTCP (DireccionRemota, PuertoRemoto));
	ProbarTransporte();
}
...
ImpresorFiscal::EstablecerSocketUDP    (const std::string &DireccionRemota,
										...
										throw(Excepcion)
{
	Protocolo->EstablecerTransporte(new SocketUDP (DireccionRemota, PuertoRemoto));
	ProbarTransporte();
}
...
//		 lo cual permite ocultar dicha fecha- solo es válido para el modelode 
//		 CF de 16 bits P435 (version 2.03) y posteriores.
//
void
ImpresorFiscal::ImprimirVoucher (const std::string &NombreCliente,
									const std::string &NombreTarjeta,
