...
SocketTCP::SocketTCP(	const std::string &DireccionRemota, 
						unsigned short PuertoRemoto,
						unsigned short PuertoLocal) throw(Excepcion)
{
#ifdef P_WIN32
	InitWinSock();
#endif
	...
	if ((SocketDescriptor = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) 
	{
		...
		Debug::Debug("SocketTCP", "Error WSASocket: " + ErrDesc);
	...
	if(bind(SocketDescriptor, (struct sockaddr *)&Name, sizeof Name) < 0)
	{
		std::string ErrDesc = Excepcion::SocketGetLastErrorDesc();
		Close();
		Debug::Debug("SocketTCP", "Error bind: " + ErrDesc);
	...
	{
		std::string ErrDesc = Excepcion::SocketGetLastErrorDesc();
		Debug::Debug("SocketTCP", "Error connect: " + ErrDesc);
		Close();
...
// Abrir puerto:
	...
	{
		std::string ErrDesc = Excepcion::SocketGetLastErrorDesc();
		Debug::Debug("SocketUDP", "Error WSASocket: " + ErrDesc);
		throw Excepcion(Excepcion::TRANSPORTE_ERROR_APERTURA, ErrDesc);
	}
	...
	if(bind(SocketDescriptor, (struct sockaddr *)&Name, sizeof Name) < 0)
	{
		std::string ErrDesc = Excepcion::SocketGetLastErrorDesc();
		Close();
		Debug::Debug("SocketUDP", "Error bind: " + ErrDesc);
		throw Excepcion(Excepcion::TRANSPORTE_ERROR_BIND_SOCKET, ErrDesc);
	}
