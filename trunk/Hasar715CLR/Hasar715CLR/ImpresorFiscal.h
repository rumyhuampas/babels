...
public:
	...
	public:
		...
		{
			unsigned i;
			for (i = 0; isdigit(AAMMDD [i]); i++)
				;

			if (AAMMDD.length () != 6 || i != 6)
				throw Excepcion(Excepcion::IMPRESOR_FISCAL_ERROR_FECHA_NO_VALIDA, "Constructor FECHA (AAMMDD)");
	...
	virtual void AbrirDF (DocumentosFiscales Tipo) throw (Excepcion) = 0;
...
// Métodos
	...
	unsigned ImpresorFiscal::xtoi (const char *num, unsigned size) const;
